...
xfuse为rclone等网盘挂载程序的拓展，实现将下载好的文件分片上传到网盘挂载的目录，在本地挂载目录实现随机读取。
...
xfuse利用FUSE（用户空间文件系统）技术，将种子文件的内容映射为虚拟文件系统。系统包含以下主要组件：

1. **元数据数据库(MetadataDB)**: 存储种子文件的结构和元数据信息
2. **FUSE模块(FS)**: 处理文件系统操作，并按需获取数据
3. **扫描器(Scanner)**: 定期扫描种子目录，检查.torrent文件在挂载目录的映射并校验完整性，如果完整则分片上传到网盘
